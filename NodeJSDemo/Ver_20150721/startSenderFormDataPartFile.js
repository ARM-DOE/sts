  ...
  fs.readFile(fileListSource, "utf8", function(err, data) {
    ...
    if (jsonObj.hasOwnProperty("FileList")) {
      ...
      process.stdout.write("=========== Right before starting the SENDER! ===========\n");
			process.stdout.write("=========== There are " + fileList.length + " files to send! =============\n");
      process.stdout.write("=========================================================\n");
      ...
      maxUploadSize = uploadSize * Math.pow(2, 10); //10 * 1024;// * 1024;
      var startByte = 0;
      var endByte = 0;
      var updateStack = [];
      ...
      // for (var i = 0; i < 1; i++) {
        ...
        var jsonMetaString = JSON.stringify(fileList[i]);
				...
				var sourceFileName = path.basename(sourceFilePath, path.extname(path.basename(sourceFilePath))); // file name without extension to create chunk names
        ...
        if (fileList[i]["FileSize"] > maxUploadSize) {
          ...
          while (endByte < fileList[i]["FileSize"]) {
						var sourceChunkName = sourceFileName + "_" + chunkNo + path.extname(path.basename(sourceFilePath));
    ...
    process.stdout.write("Caught error trying to read and send --->>> " + err + "\n");
  }
  ...
  if (res.statusCode < 399) {
    ...
    res.on("data", function(chunk) {
      ...
      switch (res.statusCode) {
        case 406:
          process.stdout.write(colors.error("Fire in the hole!!!! >>>>> " + http.STATUS_CODES[res.statusCode] + "\n"));
          ...
          process.stdout.write(colors.verbose(chunk) + "\n");
          //break;
        };
    });
    res.on("end", function(data) {
      process.stdout.write("<<<<<<< SOMETHING WENT EXTREMELY WRONG WITH THE TRANSFER >>>>>>> " + data + "\n");
    });
  };
};
